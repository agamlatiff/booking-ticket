          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
            ...
            <div className="lg:col-span-8 space-y-10">
              ...
              <div className="relative flex items-center justify-between mb-8 px-4">
                ...
                {[
                  { num: 1, label: "Layanan", active: true },
                  { num: 2, label: "Jadwal", active: true },
                  { num: 3, label: "Data Diri", active: false },
                ].map((step) => (
                  <div key={step.num} className="relative z-10 flex flex-col items-center gap-2">
                    <div className={`flex h-10 w-10 items-center justify-center rounded-full border-2 border-foreground font-bold shadow-pop ${step.active ? "bg-primary text-white" : "bg-white text-gray-400"}`}>
                      {step.num}
                    </div>
                    <span className={`text-xs font-bold bg-background px-1 ${step.active ? "text-foreground" : "text-gray-400"}`}>{step.label}</span>
                  </div>
                ))}
              </div>
