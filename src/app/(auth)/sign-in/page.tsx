...
export default function SignInPage() {
  ...
  const handleCredentialsSubmit = async (formData: FormData) => {
    ...
    try {
      ...
      // Redirect will throw NEXT_REDIRECT error, which is expected
      setIsLoading(false);
    }
  };
    ...
    <div className="flex flex-col min-h-screen bg-background font-sans">
      ...
      <main className="flex-grow flex items-center justify-center relative pt-32 pb-20 px-6 overflow-hidden">
        ...
        <div className="absolute left-[-5%] top-40 opacity-40 pointer-events-none">
          <svg className="text-accent-yellow animate-[bounce_5s_infinite]" fill="currentColor" height="200" viewBox="0 0 200 200" width="200">
        ...
        <div className="absolute right-[-2%] bottom-10 opacity-30 pointer-events-none">
          <svg className="text-primary animate-[bounce_4s_infinite]" fill="currentColor" height="150" viewBox="0 0 200 200" width="150">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-0 lg:gap-8 rounded-[2.5rem] bg-white border-2 border-foreground shadow-card overflow-hidden">
            ...
            <div className="p-8 md:p-12 lg:p-14 flex flex-col justify-center order-2 lg:order-1">
              <div className="mb-8">
                ...
                <h1 className="font-display text-4xl font-black text-foreground mb-2">Masuk Akun</h1>
                <p className="text-gray-600">Akses jadwal dan riwayat perawatanmu dengan mudah.</p>
              </div>
              ...
              <form action={signInWithGoogle}>
                <button
                  type="submit"
                  className="w-full flex items-center justify-center gap-3 py-3 rounded-xl font-bold mb-6 bg-white border-2 border-foreground shadow-pop hover:shadow-pop-hover hover:translate-x-[2px] hover:translate-y-[2px] transition-all"
                >
              ...
              <div className="relative mb-6">
                ...
                <div className="relative flex justify-center text-sm">
                  <span className="bg-white px-4 text-gray-500 font-medium">atau dengan email</span>
                </div>
              </div>

              {/* Credentials Form */}
              <form action={handleCredentialsSubmit} className="space-y-5">
                ...
                <div className="flex items-center justify-between text-sm">
                  <label className="flex items-center gap-2 cursor-pointer group">
                    ...
                    <span className="text-gray-600 font-medium group-hover:text-foreground">Ingat saya</span>
                  </label>
                  <a className="text-primary font-bold hover:underline" href="#">Lupa Password?</a>
                </div>
                <button
                  className="w-full rounded-xl bg-primary py-3.5 text-white font-bold text-lg mt-2 border-2 border-foreground shadow-pop hover:shadow-pop-hover hover:translate-x-[2px] hover:translate-y-[2px] transition-all flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
                  type="submit"
                  disabled={isLoading}
                >
                  {isLoading ? (
              ...
              <p className="mt-8 text-center text-gray-600 text-sm font-medium">
                Belum punya akun?
                <Link className="text-secondary font-bold hover:underline ml-1" href="/sign-up">Daftar disini</Link>
              </p>
            </div>
